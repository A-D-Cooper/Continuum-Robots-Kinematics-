...
int main(int argc, char **argv)
{
	...
	//Create controller
	int control_scenario = 0;
	Controller controller(&tdcr_model,&ctcr_model);
	...
	else if(assignment == 4)
	{
		...
		std::cin >> control_scenario;
		
		std::cout << std::endl;
		
		if(control_scenario == 0)
		{
			Eigen::Matrix<double,9,1> q;
			q << -0.005,
				 0.0025,
		...
		{
			std::cout << "Invalid scenario chosen!" << std::endl; 
			return 0;
		}
	}
	else
	...
	//Create Main Loop
	MainLoop eventLoop(&vis, &controller, &tdcr_model, &ctcr_model, timestep, assignment, control_scenario);
